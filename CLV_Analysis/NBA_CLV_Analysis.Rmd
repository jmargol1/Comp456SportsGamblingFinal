...
NBA_cv10 <- vfold_cv(NBA_Home, v = 10)
...
# Recipe
logistic_rec <- recipe(make_bet ~ money_line + spread + total + team_total, data = NBA_Home, family = binomial('logit'), maxit = 100) %>%
...
log_fit <- fit(log_wf, data = NBA_Home)
```
...
predict(log_fit, new_data = NBA_Home, type = "prob")

# Make hard (class) predictions (using a default 0.5 probability threshold)
predict(log_fit, new_data = NBA_Home, type = "class")
```
...
```{r}
# Soft predictions
logistic_output <-  NBA_Home %>%
  bind_cols(predict(log_fit, new_data = NBA_Home, type = 'prob')) 
...
data_fit_mtry2 <- fit(data_wf_mtry2, data = NBA_Home)
...
set.seed(121) 
data_fit_mtry4 <- fit(data_wf_mtry4, data = NBA_Home)

"set.seed(121)
data_fit_mtry6 <- fit(data_wf_mtry6, data = NBA_Home)"
```
...
#rf_OOB_output(data_fit_mtry6,6, NBA_Home %>% pull(make_bet))
rf_OOB_output(data_fit_mtry4,4, NBA_Home %>% pull(make_bet))
rf_OOB_output(data_fit_mtry2,2, NBA_Home %>% pull(make_bet))
```
...
NBA_cv10 <- vfold_cv(NBA_Away, v = 10)
...
# Recipe
logistic_rec <- recipe(make_bet ~ money_line + spread + total + team_total, data = NBA_Away, family = binomial('logit'), maxit = 100) %>%
    step_normalize(all_numeric_predictors()) %>% 
    step_dummy(all_nominal_predictors())
...
# Fit Model to Training Data
log_fit <- fit(log_wf, data = NBA_Away)
```
...
```{r}
# Make soft (probability) predictions
predict(log_fit, new_data = NBA_Away, type = "prob")

# Make hard (class) predictions (using a default 0.5 probability threshold)
predict(log_fit, new_data = NBA_Away, type = "class")
```
...
```{r}
# Soft predictions
logistic_output <-  NBA_Away %>%
  bind_cols(predict(log_fit, new_data = NBA_Away, type = 'prob')) 
...
set.seed(121) # make sure to run this before each fit so that you have the same 1000 trees
data_fit_mtry2 <- fit(data_wf_mtry2, data = NBA_Away)
...
# Fit models for 12, 74, 147
set.seed(121) 
data_fit_mtry4 <- fit(data_wf_mtry4, data = NBA_Away)
...
#check out the function output
#rf_OOB_output(data_fit_mtry6,6, NBA_Home %>% pull(make_bet))
rf_OOB_output(data_fit_mtry4,4, NBA_Away %>% pull(make_bet))
rf_OOB_output(data_fit_mtry2,2, NBA_Away %>% pull(make_bet))
```
...
ct_spec <- decision_tree() %>%
  set_engine(engine = 'rpart') %>%
  set_args(cost_complexity = 0.003,  #default is 0.01 (used for pruning a tree)
           min_n = NULL, #min number of observations to try split: default is 20 [I think the documentation has a typo and says 2]  (used to stop early)
           tree_depth = NULL) %>% #max depth, number of branches/splits to get to any final group: default is 30 (used to stop early)
  set_mode('classification') # change this for regression tree
