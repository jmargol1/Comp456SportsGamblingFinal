...
NHL_cv10 <- vfold_cv(NHL_Home, v = 10)
...
# Recipe
logistic_rec <- recipe(make_bet ~ money_line + spread + total + team_total, data = NHL_Home, family = binomial('logit'), maxit = 100) %>%
...
log_fit <- fit(log_wf, data = NHL_Home)
```
...
predict(log_fit, new_data = NHL_Home, type = "prob")

# Make hard (class) predictions (using a default 0.5 probability threshold)
predict(log_fit, new_data = NHL_Home, type = "class")
```
...
```{r}
# Soft predictions
logistic_output <-  NHL_Home %>%
  bind_cols(predict(log_fit, new_data = NHL_Home, type = 'prob')) 
...
data_fit_mtry2 <- fit(data_wf_mtry2, data = NHL_Home)
...
set.seed(121) 
data_fit_mtry4 <- fit(data_wf_mtry4, data = NHL_Home)

"set.seed(121)
data_fit_mtry6 <- fit(data_wf_mtry6, data = NHL_Home)"
```
...
#rf_OOB_output(data_fit_mtry6,6, NHL_Home %>% pull(make_bet))
rf_OOB_output(data_fit_mtry4,4, NHL_Home %>% pull(make_bet))
rf_OOB_output(data_fit_mtry2,2, NHL_Home %>% pull(make_bet))
```
...
NHL_cv10 <- vfold_cv(NHL_Away, v = 10)
...
# Recipe
logistic_rec <- recipe(make_bet ~ money_line + spread + total + team_total, data = NHL_Away, family = binomial('logit'), maxit = 100) %>%
    step_normalize(all_numeric_predictors()) %>% 
    step_dummy(all_nominal_predictors())
...
# Fit Model to Training Data
log_fit <- fit(log_wf, data = NHL_Away)
```
...
```{r}
# Make soft (probability) predictions
predict(log_fit, new_data = NHL_Away, type = "prob")

# Make hard (class) predictions (using a default 0.5 probability threshold)
predict(log_fit, new_data = NHL_Away, type = "class")
```
...
```{r}
# Soft predictions
logistic_output <-  NHL_Away %>%
  bind_cols(predict(log_fit, new_data = NHL_Away, type = 'prob')) 
...
set.seed(121) # make sure to run this before each fit so that you have the same 1000 trees
data_fit_mtry2 <- fit(data_wf_mtry2, data = NHL_Away)
...
# Fit models for 12, 74, 147
set.seed(121) 
data_fit_mtry4 <- fit(data_wf_mtry4, data = NHL_Away)
...
#check out the function output
#rf_OOB_output(data_fit_mtry6,6, NHL_Home %>% pull(make_bet))
rf_OOB_output(data_fit_mtry4,4, NHL_Away %>% pull(make_bet))
rf_OOB_output(data_fit_mtry2,2, NHL_Away %>% pull(make_bet))
```
